...
struct _exactcover {
  ...
  _exactcover(const vvb& mat, T& callback_) : w(mat[0].size()), h(mat.size()), callback(callback_) {
    root = getnode();
    root->left = root;
    root->right = root;
    root->name = -1000000;
  ...
  void solve(void) {
    ...
    for (node* r = mincol->down; r != mincol; r = r->down) {
      for (node* j = r->right; j != r; j = j->right) cover(j->top);
      solution.push_back(r->name);
      solve();
      solution.pop_back();
      for (node* j = r->left; j != r; j = j->left) uncover(j->top);
    }
    uncover(mincol);
  }
};
